...
SELECT first_name, last_name, hire_date, birth_date, DATEDIFF(CURDATE(),hire_date) / 365
AS working_years 
FROM employees
WHERE birth_date LIKE '%12-25%' 
AND hire_date BETWEEN '1990-01-01' AND '1999-12-31';
...
-- creating a username: 
-- all lowercase-> LOWER()
-- first letter of first name -> SUBSTR()
-- first four of last name -> SUBSTR()
-- '-' -> CONCAT()
-- month of birth -> SUBSTR() 
-- last two digits of birth year -> SUBSTR() 
